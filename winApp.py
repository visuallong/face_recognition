...
classify_methods_chooser = ['Cosine Similarity']
face_detect_methods_chooser = ['SSD Mobilenet']
activate_face_detect_method = 'SSD Mobilenet'
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
            ...
            button2 = tk.Button(self, text="  Face Recognitize ", fg="#ffffff", bg="#263942",command=lambda:self.controller.show_frame("PageTwo"))
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button3 = tk.Button(self, text=" Add list user ", fg="#ffffff", bg="#263942",command=self.add_list_user)
        button4 = tk.Button(self, text="Back", bg="#ffffff", fg="#263942",command=lambda:self.controller.show_frame("StartPage"))
        button1.grid(row=1, column=0, ipady=3, ipadx=1)
        button2.grid(row=2, column=0, ipady=3, ipadx=1)
        button3.grid(row=3, column=0, ipady=3, ipadx=9)
        button4.grid(row=4, column=0, ipady=3, ipadx=31)

    def add_list_user(self):
        folder_path = askdirectory()
        if folder_path != '':
            k = 0
            for sub_fld_name in os.listdir(folder_path):
                sub_fld_path = r"{}/{}".format(folder_path, sub_fld_name)
                if os.path.isdir(sub_fld_path):
                    ...
                    fld_list.append(t)
                    count = 0
                    for file in os.listdir(sub_fld_path):
                        ...
                        if file_path.lower().endswith(('.png', '.jpg', '.jpeg')):
                            ...
                            if faces:
                                i = 0
                                for face in faces:
                                    if len(os.listdir(save_fld_path)) >= 8:
                                        print('User {} have enought 8 face images'.format(sub_fld_name))
                                    else:
                                        save_file_path = os.path.join(save_fld_path, "img-" + time.strftime("%d-%m-%y-%H-%M-%S") + ".jpg")
                                        if os.path.exists(save_file_path):
                                            f_name, f_ext = os.path.splitext(save_file_path)
                                            save_file_path = f_name + '_' + str(i) + f_ext
                                            i += 1
                                        cv2.imwrite(save_file_path, face)
                                        count += 1
                    print('Add {} face(s) of user {}'.format(count, sub_fld_name))
                    # if len(os.listdir(save_fld_path)) == 0:
                    #     os.rmdir(save_fld_path)`
            self.controller.update_json_file()
            self.controller.frames["PageFour"].refresh_names()
            print('Add list user done')
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label.grid(row=0, column=0)
        # tk.Label(self, text="Choose method use in recognitize:").grid(row=1)
        tk.Label(self, text="Choose method use in recognitize:").grid_forget()
...
class PageThree(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.rowconfigure(1, minsize=190)
        ...
        self.train_btn.grid(row=0, column=0, columnspan=2)
        ...
        self.batch_size_var = tk.IntVar(value=24)
        ...
        self.defaults_label.grid(row=3, column=0, columnspan=2)
        ...
        # self.underthehood_label.grid(row=4, column=0, columnspan=2)
        self.underthehood_label.grid_forget()
        ...
        tk.Label(self.tab3, text="Choose method to train:").grid(row=0)
        self.activate_classify_method = 'SVM'
        self.method_classify_var = tk.StringVar(value=self.activate_classify_method)
        self.method_classify_train_menu = ['SVM', 'KNN']
        self.methods_menu = tk.OptionMenu(self.tab3, self.method_classify_var, *self.method_classify_train_menu, command=self.train_classify_view)
...
class PageFour(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        self.columnconfigure(0, minsize=165)
        self.columnconfigure(1, minsize=165)
    ...
    def view(self):
        ...
        self.scrollbar.grid(row=3, column=0, columnspan=3)
...
class PageSix(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.columnconfigure(0, minsize=125)
        self.columnconfigure(1, minsize=125)
        self.columnconfigure(2, minsize=125)
        self.columnconfigure(3, minsize=125)
        ...
        self.rowconfigure(5, minsize=25)
        self.webcam_ = None
        self.controller = controller
        self.parent = parent
        self.add_img = tk.PhotoImage(file=r'storage/something/add.png')
        ...
        self.btn_show_more = tk.Menubutton(self, image=self.show_more_img, relief="raised")
        self.btn_show_more.grid(row=0,column=2)
        self.btn_show_more.menu = tk.Menu(self.btn_show_more, tearoff = 0)
        self.btn_show_more['menu'] = self.btn_show_more.menu
        self.btn_show_more.menu.add_command(label="Delete User", command=lambda:self.delete_user())
        self.btn_show_more.menu.add_command(label="Remove All Samples", command=lambda:self.remove_all_samples())
...
class webcam(tk.Toplevel):
    def __init__(self, parent, controller):
        ...
        try:
            ...
            self.destroy()
            messagebox.showerror('Error', "Unable to open this camera \n Select another video source")
            return None
        self.columnconfigure(0, weight=1)
        ...
        tk.Label(self,text="Choose method to detect face:").grid_forget()
        global activate_face_detect_method
        ...
        self.detect_method_menu.grid_forget()
        self.update()
